...
<div class="row">
    ...
    <div class="box box-success color-palette-box">
        <div class="box-header with-border">
          <h3 class="box-title" style="color: #00a65a"><i class="fa fa-bars"></i> Number in analysis: {{$target}}</h3>
        </div>
        ...
        <table class="table table-bordered">
                ...
                <tr>
                  ...
                  <th>Number</th>
                  <th>Co presence count</th>
                  <th>Date, Time, Address</th>
                </tr>
                @foreach($alldata as $keyindex=> $value)
                <tr>
                  <td>{{$keyindex}}</td>
                  <td>{{$value['aparty']}}</td>
                  <td>{{$value['count']}}</td>
                  <td>
                  @foreach($value['dates'] as $moment)
                  {{$moment->date}} || {{$moment->time}} || {{$moment->address}}<br \>
                  @endforeach
                  </td>
                </tr>
                @endforeach
